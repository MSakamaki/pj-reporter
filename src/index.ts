...
export async function GeneratePDF(suite: JasmineSuite, options: GeneratePdfOptions) {
  ...
  try {
    ...
    const pdfOptions: PDFOptions = {
      ...
      height: `${config.display.height + config.headerHeight + config.footerHeight}px`,
      path: filepath,
      scale: config.scale,
